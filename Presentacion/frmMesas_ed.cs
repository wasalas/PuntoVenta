...
{
    public partial class frmMesas_ed : Form
    {
        #region "Mis Variables"
        private int Estado_guarda;
        private EMesas oDatos;
        public bool GraboDatos = false;
        #endregion

        #region "Metodos del Form"
        public frmMesas_ed(int Opcion, EMesas Datos)
        {
            InitializeComponent();
            this.Estado_guarda = Opcion;
            this.oDatos = Datos;
        }
        private void frmMesas_ed_Load(object sender, EventArgs e)
        {
            ...
            {
                this.txt_codigo.Text = oDatos.Codigo_me.ToString();
                this.txt_descrip.Text = oDatos.Descripcion_me;
        ...
        private void btn_guardar_Click(object sender, EventArgs e)
        {
            ...
            oDatos.Codigo_me = Convert.ToInt32(this.txt_codigo.Text);
            oDatos.Descripcion_me = Convert.ToString(this.txt_descrip.Text.Trim().ToUpper());
            oDatos.Codigo_pv = Convert.ToInt32(this.txt_codigo_pv.Text);
            oDatos.Estado = Convert.ToByte(this.chk_estado.Checked ? 1 : 0);

            if (oDatos.Descripcion_me == String.Empty)
            {
            ...
            {
                Rpta = NMesas.Guardar(this.Estado_guarda, this.oDatos);
                if (Rpta == "OK")
                {
