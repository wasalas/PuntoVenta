...
{
    public partial class frmTurnos_ed : Form
    {
        #region "Mis Variables"
        private int Estado_guarda;
        private ETurnos oDatos;
        public bool GraboDatos = false;
        #endregion

        #region "Metodos del Form"
        public frmTurnos_ed(int Opcion, ETurnos Datos)
        {
            InitializeComponent();
            this.Estado_guarda = Opcion;
            this.oDatos = Datos;
        }
        private void frmTurnos_ed_Load(object sender, EventArgs e)
        {
            ...
            {
                this.txt_codigo.Text = oDatos.Codigo_tu.ToString();
                this.txt_descrip.Text = oDatos.Descripcion_tu;
        ...
        private void btn_guardar_Click(object sender, EventArgs e)
        {
            ...
            oDatos.Codigo_tu = Convert.ToInt32(this.txt_codigo.Text);
            oDatos.Descripcion_tu = Convert.ToString(this.txt_descrip.Text.Trim().ToUpper());
            oDatos.Estado = Convert.ToByte(this.chk_estado.Checked ? 1 : 0);

            if (oDatos.Descripcion_tu == String.Empty)
            {
            ...
            {
                Rpta = NTurnos.Guardar(this.Estado_guarda, this.oDatos);
                if (Rpta == "OK")
                {
